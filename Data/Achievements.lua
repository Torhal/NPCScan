...
local AchievementID = {
    ...
    AdventurerOfThaldraszus = 16679,
    AdventurerOfTheAzureSpan = 16678,
    AdventurerOfTheEmeraldDream = 19316,
    AdventurerOfTheIsleOfDorn = 40435,
    AdventurerOfTheOhnahranPlains = 16677,
    AdventurerOfTheRingingDeeps = 40837,
    AdventurerOfTheWakingShores = 16676,
    AdventurerOfTiragardeSound = 12939,
    AdventurerOfValsharah = 11262,
    AdventurerOfVoldun = 12943,
    AdventurerOfZaralekCavern = 17783,
    AdventurerOfZerethMortis = 15391,
...
local function TryAssignNPCToAchievement(
    ...
    if not foundMatch then
        private.Debug(
            "** AchievementID.%s - (criteriaID %s): %s = %d, -- %s",
            achievementLabel,
            achievementCriteriaID,
            npcDataField,
            achievementAssetID,
...
local function InitializeAchievements()
    for achievementLabel, achievementID in pairs(AchievementID) do
        ...
        for criteriaIndex = 1, GetAchievementNumCriteria(achievementID) do
            local assetName, criteriaType, isCriteriaCompleted, _, _, _, _, assetID, _, criteriaID =
            ...
            if criteriaType == CriteriaType.NPCKill then
                if assetID > 0 then
                    ...
                    if found then
                        ...
                        private.Debug(
                            "** AchievementID.%s - (criteriaID %d): NPC %s with assetID %d",
                            achievementLabel,
                            criteriaID,
                            assetName,
                            assetID
                ...
                if dataField then
                    ...
                    private.Debug(
                        "** AchievementID.%s: Unknown criteria type %d, assetID %d",
                        achievementLabel,
                        criteriaType,
                        assetID
                    )
